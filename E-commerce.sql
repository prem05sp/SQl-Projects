...
select count(*) from users_data; -- in user_data could not import all 98913 rows i dont know why i try many times only import 89672 only.
...
select gender,count(socialNbFollowers) from users_data group by gender;
...
/* 7.Calculate the total users those
 ...
 select country,count(productssold) sellers from users_data where productssold > 0 group by country order by sellers desc;
 ...
 select * from users_data;
 select country,count(productspassrate) as Product from users_data where productspassrate > 0
...
select * from users_data;
select country, max(productsBought) Buyers from users_data where productsBought > 0 
